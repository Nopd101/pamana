        ...
        >
          ...
          {activeTab === "quiz" && (
            <div className="space-y-8">
              <div>
                <h2 className="text-3xl font-black text-[#5a2d0c] font-[var(--font-heading)] uppercase">{currentData.quizTitle}</h2>
                <p className="text-[#B06A3A] font-bold leading-relaxed">{currentData.quizInstructions}</p>
              </div>
                        ...
                        {hanayA.map((item) => {
                          ...
                          return (
                            <div
                              ...
                              style={{ backgroundColor: isConnected ? "#772402" : item.color }}
                              className={`p-4 rounded-lg font-bold text-center text-sm min-h-[100px] flex items-center justify-center cursor-pointer transition-all border-4 ${selectedA === item.id ? "border-amber-400 scale-105 shadow-lg z-20" : "border-transparent"} text-white`}
                            >
                              {item.text}
                            </div>
                          );
                      ...
                      <div className="space-y-4">
                        ...
                        {hanayB.map((text, i) => (
                          <button
                            ...
                            className={`w-full border-2 border-[#772402] p-4 md:p-6 rounded-lg font-black uppercase transition-colors min-h-[60px] ${connections.find((c) => c.toId === `b${i}`) ? "bg-[#772402] text-white" : "text-[#772402] hover:bg-amber-50"}`}
                          >
                            {text}
                          </button>
                        ))}
              ...
              {id === "mesoamerica" && activeTab === "quiz" && (
                <div className="space-y-6">
                  <div className="space-y-4">
                    {mesoQuestions.map((q) => (
                      <div key={`${resetKey}-${q.id}`} className="border-2 border-[#5a2d0c]/30 rounded-xl p-6 bg-white shadow-sm">
              ...
              <div className="flex flex-col md:flex-row justify-end pt-4 gap-4">
                <button onClick={handleClearAnswers} className="w-full md:w-auto border-2 border-[#772402] text-[#772402] px-8 py-3 rounded-xl font-black text-lg shadow-md hover:bg-amber-50 transition-all active:scale-95">
                  Clear Answer
                </button>
                <button onClick={handleSubmitQuiz} className="w-full md:w-auto bg-[#772402] text-white px-12 py-3 rounded-xl font-black text-xl shadow-xl hover:bg-[#5a2d0c] transition-all transform active:scale-95">
                  Submit Answer
                </button>
              </div>
            </div>
